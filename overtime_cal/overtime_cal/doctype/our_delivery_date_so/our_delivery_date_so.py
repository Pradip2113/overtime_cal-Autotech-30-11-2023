...
# from frappe import ignore_permissions

class OurDeliveryDateSO(Document):
	@frappe.whitelist()
	def opcost(self):
	...
	# @frappe.whitelist()
	# def setval(self):
	# 	doc=frappe.db.get_list('Sales Order Item',filters={'parent':self.sales_order_no},fields={"name","parent","our_delivery_date","item_code"},ignore_permissions=True)
	# 	for i in self.get("item"):
	# 		for d in doc:
	# 			if i.items == d.item_code:
	# 				with ignore_permissions():
	# 					frappe.db.set_value("Sales Order Item", d.name, "our_delivery_date",i.our_delivery_date)
